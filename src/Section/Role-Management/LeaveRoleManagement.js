...
const LeaveRoleManagement = () => {
  ...
  const getLeaveRole = () => {
    DataService.getLeaveRole()
      .then((data) => {
        const fetchedRoles = data.data.data?.ApprovelRole || [];
        setData(data.data.data);
        setSelectedRoles(fetchedRoles);
        setAllChecked(fetchedRoles.length === 3);
        setSomeChecked(fetchedRoles.length > 0 && fetchedRoles.length < 3);
      })
      .catch((error) => {
        const resMessage =
          (error.response &&
  ...
  const handleSubmit = (roles) => {
    ...
    DataService.updateApproveRole(data).then(
      () => {
  ...
  const handleIndividualCheckbox = (role) => {
    setSelectedRoles((prev) => {
      ...
      setAllChecked(newRoles.length === 3);
      setSomeChecked(newRoles.length > 0 && newRoles.length < 3);
      return newRoles;
    });
  };
  ...
  const handleSelectAll = () => {
    ...
    const newRoles = newCheckedState ? ["admin", "HR", "TL"] : [];
...
export default LeaveRoleManagement;
