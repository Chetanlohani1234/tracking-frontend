...
const ReceiveDirectGRN = () => {
  ...
  useEffect(() => {
    ...
    if (id) {
      DataService.directGetGrn(id)
        .then((response) => {
          const po = response.data;
          console.log("wdefsdfs",po);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (id) {
        // Update existing GRN
        await DataService.UpdateGrn(id, data);
  ...
  console.log("sdsdsad",orderItems);
  ...
  return (
    <>
      ...
      <div className="po-container">
        <h2 className="po-title">{id ? 'Update GRN' : 'Purchase Order Form'}</h2>
...
export default ReceiveDirectGRN;
